...
interface AnswerOptionsProps {
  question: Question;
  onAnswerSelect: (answerId: string, timeToAnswer: number) => void;
...
export function AnswerOptions({ 
  ...
  const handleAnswerClick = (answerId: string) => {
    ...
    onAnswerSelect(answerId, timeToAnswer);
  };

  const getOptionColor = (option: AnswerOption, isSelected: boolean) => {
    const baseColors = {
      red: 'bg-red-500',
      blue: 'bg-blue-500', 
    ...
    if (isAnswered) {
      ...
      if (isSelected && option.id !== question.correctAnswer) {
    ...
    if (isSelected) {
      return selectedColors[option.color];
    }

    return `${baseColors[option.color]} ${hoverColors[option.color]}`;
  };
      ...
      {question.options.map((option) => (
        <button
          ...
          className={`
            ...
            ${getOptionColor(option, selectedAnswer === option.id)}
          `}
        >
